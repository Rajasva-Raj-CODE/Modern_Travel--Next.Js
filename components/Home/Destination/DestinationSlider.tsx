            ...
            {destinationData.map((data) => {
                return <div key={data.id} className='m-3'>
                    <div className='relative h-[400px]'>
                        ...
                        {/*Image*/}
                        <Image src={data.image} alt={data.country} width={500} height={500} className='rounded-lg h-full w-full object-cover' />
                    </div>
                    {/*Text containet*/}
                    <h1 className='mt-4 text-lg font-semibold'>{data.country}</h1>
